...
import com.framework.api.pojos.TableData;

public class CodeHelper {
	public static String MAIN_PACKAGE = "com.framework.api";
	public static String MAIN_PACKAGE_PATH = "src/main/java/com/framework/api/";

	public static void main(String[] args) throws IOException {
		System.out.println("Do you really want to create CRUD?");
		BufferedReader serviceInput = new BufferedReader(new InputStreamReader(System.in));
		String dbOverride = serviceInput.readLine();
		if (dbOverride.equalsIgnoreCase("y")) {
			List<TableData> tableList = new ArrayList<>();
			tableList.add(new TableData("test", "Long", "", true, false));
					...
					if (linePre.trim().endsWith("}")) {
						if (!builder.toString().contains(fileName + "s Validations")) {
							...
							builder.append("\tpublic static void validate" + fileName + "Request(" + fileName
									+ "Request requests) throws CustomException {\n");
							for (TableData entry : fieldsMap) {
								if (entry.isNotNull()) {
									if (entry.getType().equals("String")) {
										builder.append("\tif(StringUtils.isEmpty(requests.get"
										...
										builder.append("\tif(CollectionUtils.isEmpty(requests.get"
												+ getUppercase(entry.getName()).charAt(0)
												+ entry.getName().substring(1, entry.getName().length()) + "())){\n");
									} else {
										builder.append("\tif(requests.get" + getUppercase(entry.getName()).charAt(0)
												+ entry.getName().substring(1, entry.getName().length())
												+ "()==null){\n");
									}
									builder.append("\t\tthrow new CustomException(CustomApiResponseCode.EMPTY_"
							...
							builder.append("\tpublic static void validate" + fileName + "Response(" + fileName
									+ "Response requests) throws CustomException {\n");
							builder.append("\tif(StringUtils.isEmpty(requests.get" + fileName + "Id())){\n");
							builder.append("\t\tthrow new CustomException(CustomApiResponseCode.EMPTY_"
	...
	private static void addErrorConstants(String filePath, String fileName, List<TableData> fieldsMap) {
		try {
			FileReader fileReader = new FileReader(filePath + "CustomApiResponseCode.java");
			BufferedReader bufferedReader = new BufferedReader(fileReader);
			StringBuilder builder = new StringBuilder();
			String linePre = "";
			String line;
			...
			bufferedReader.close();
			FileOutputStream out = new FileOutputStream(filePath + "CustomApiResponseCode.java");
			out.write(builder.toString().getBytes());
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	private static void createController(String folder, String fileName, List<TableData> fieldsMap) {
		try {
			...
			fileWriter.write("import " + MAIN_PACKAGE + ".enums.CustomApiResponseCode;\n");
			fileWriter.write("import " + MAIN_PACKAGE + ".exception.CustomException;\n");
			...
			fileWriter.write("\tpublic BaseApiResponse<" + fileName
					+ "Request> generateRequestJson() throws CustomException{\n");
			fileWriter.write("\treturn new BaseApiResponse<" + fileName
					+ "Request>(false, CustomApiResponseCode.SUCCESS.getCode(), " + fileName
					+ "Converter.getSample(),CustomApiResponseCode.SUCCESS.getMessage());\n");
			fileWriter.write("\t}\n\n");
			...
			fileWriter.write("@RequestBody(required = true) " + fileName + "Request requests");
			fileWriter.write(")throws CustomException{\n");
			fileWriter.write("\tValidationUtils.validate" + fileName + "Request(requests);\n");
			for (TableData tableData : fieldsMap) {
				if (tableData.isUnique()) {
					fileWriter.write("\tif(" + serviceName + ".existBy" + getMethodeName(tableData.getName())
							+ "(requests.get" + getMethodeName(tableData.getName()) + "())){\n");
					fileWriter.write("\t\tthrow new CustomException(CustomApiResponseCode.EXIST_"
							+ getUppercase(tableData.getName()) + ");\n");
					fileWriter.write("\t}\n");
				}
			}
			fileWriter.write("\tLong responses = " + serviceName + ".add(requests);\n");
			fileWriter.write(
			...
			fileWriter.write("@PathVariable(RestMappingConstants.ID) Long id)throws CustomException{\n");
			fileWriter.write("\t" + fileName + "Response responses = " + serviceName + ".getById(id);\n");
			fileWriter.write("\tif(responses==null){\n");
			fileWriter.write(
					"\t\tthrow new CustomException(CustomApiResponseCode." + fileName.toUpperCase() + "_NOT_FOUND);\n");
			fileWriter.write("\t}\n");
			fileWriter.write("\treturn new BaseApiResponse<" + fileName
					+ "Response>(false, CustomApiResponseCode.SUCCESS.getCode(), responses, CustomApiResponseCode.SUCCESS.getMessage());\n");
			fileWriter.write("\t}\n\n");
			...
			fileWriter.write("\t@ResponseBody\n");
			fileWriter.write(
					"\tpublic BaseApiResponse<List<" + fileName + "Response>> getAll()throws CustomException{\n");
			fileWriter.write("\tList<" + fileName + "Response> responses = " + serviceName + ".getAll();\n");
			fileWriter.write("\tif(CollectionUtils.isEmpty(responses)){\n");
			fileWriter.write(
					"\t\tthrow new CustomException(CustomApiResponseCode.NO_" + fileName.toUpperCase() + "S_FOUND);\n");
			fileWriter.write("\t}\n");
			...
			fileWriter.write("\t@ResponseBody\n");
			fileWriter
					.write("\tpublic BaseApiResponse<Boolean> update(@PathVariable(RestMappingConstants.ID) Long id,");
			fileWriter
					.write("@RequestBody(required = true) " + fileName + "Response requests)throws CustomException{\n");
			fileWriter.write("\tif(!" + serviceName + ".exist(id)){\n");
			fileWriter.write(
					"\t\tthrow new CustomException(CustomApiResponseCode." + fileName.toUpperCase() + "_NOT_FOUND);\n");
			fileWriter.write("\t}\n");
			fileWriter.write("\trequests.set" + fileName + "Id(id);\n");
			fileWriter.write("\tBoolean responses = " + serviceName + ".update(requests);\n");
			fileWriter.write(
	...
	private static void createServiceImpl(String folder, String fileName, List<TableData> fieldsMap) {
		try {
			...
			fileWriter.write("\tpublic Long add(" + fileName + "Request requests){\n");
			fileWriter.write("\t" + fileName + " entity=" + fileName + "Converter.getEntityFromRequest(requests);\n");
			...
			fileWriter.write("\t@Override\n");
			fileWriter.write("\tpublic boolean update(" + fileName + "Response requests){\n");
			fileWriter.write("\t" + fileName + " entity=" + daoName + ".getTById(requests.get" + fileName + "Id());\n");
			fileWriter.write("\t" + fileName + "Converter.getEntityFromResponse(requests,entity);\n");
			fileWriter.write("\tif(entity!=null){\n");
	...
	private static void createService(String folder, String fileName, List<TableData> fieldsMap) {
		try {
			...
			fileWriter.write("Service{\n");
			fileWriter.write("\tLong add(" + fileName + "Request requests);\n");
			...
			fileWriter.write("\tboolean update(" + fileName + "Response requests);\n");
			for (TableData data : fieldsMap) {
				if (data.isUnique()) {
	...
	private static void createConverter(String folder, String fileName, List<TableData> fieldsMap) {
		try {
			...
			fileWriter.write("\t\t" + fileName + "Request responses = new " + fileName + "Request();\n");
			for (TableData entry : fieldsMap) {
				System.out.println("name:" + entry.getName());
				fileWriter.write("\t\tresponses.set" + getMethodeName(entry.getName()) + "("
						+ getSample(entry.getType(), entry.getName()) + ");\n");
			}
			fileWriter.write("\t\treturn responses;\n");
			fileWriter.write("\t}\n");

			fileWriter.write(
					"\n\tpublic static " + fileName + " getEntityFromRequest(" + fileName + "Request requests) {\n");
			fileWriter.write("\t\tif(requests!=null){\n");
			fileWriter.write("\t\t\t" + fileName + " responses = new " + fileName + "();\n");
			for (TableData entry : fieldsMap) {
				fileWriter.write("\t\t\tresponses.set" + getMethodeName(entry.getName()) + "(requests.get"
			...
			fileWriter.write(
					"\n\tpublic static " + fileName + "Response getResponseFromEntity(" + fileName + " requests) {\n");
			fileWriter.write("\t\tif(requests!=null){\n");
			fileWriter.write("\t\t\t" + fileName + "Response responses = new " + fileName + "Response();\n");
			fileWriter.write("\t\t\tresponses.set" + fileName + "Id(requests.getId());\n");
			for (TableData entry : fieldsMap) {
				fileWriter.write("\t\t\tresponses.set" + getMethodeName(entry.getName()) + "(requests.get"
						+ getMethodeName(entry.getName()) + "());\n");
			}
			...
			fileWriter.write("\n\tpublic static " + fileName + " getEntityFromResponse(" + fileName
					+ "Response requests," + fileName + " responses) {\n");
			fileWriter.write("\t\tif(requests!=null){\n");
			for (TableData entry : fieldsMap) {
				fileWriter.write("\t\t\tresponses.set" + getMethodeName(entry.getName()) + "(requests.get"
						+ getMethodeName(entry.getName()) + "());\n");
			...
			fileWriter.write("\n\tpublic static List<" + fileName + "Response> getResponseListFromEntityList(List<"
					+ fileName + "> requestsList) {\n");
			fileWriter.write("\t\tif(CollectionUtils.isNotEmpty(requestsList)){\n");
			fileWriter.write("\t\tList<" + fileName + "Response> responsesList = new ArrayList<>();\n");
			fileWriter.write("\t\tfor(" + fileName + " requests:requestsList){\n");
			fileWriter.write("\t\t\tresponsesList.add(getResponseFromEntity(requests));\n");
			fileWriter.write("\t\t}\n");
			fileWriter.write("\t\treturn responsesList;\n");
			fileWriter.write("\t\t}\n");
			fileWriter.write("\t\treturn null;\n");
			fileWriter.write("\t}\n");
		...
		try {
			...
			for (TableData tableData : fieldsMap) {
				if (tableData.isUnique()) {
					...
					fileWriter.write("\t\treturn getT(FROM_ENTITY + \" where \" + " + fileName + ".Columns."
							+ getUppercase(tableData.getName()) + " + \"='\" + " + tableData.getName() + "+\"'\");\n");
					fileWriter.write("\t}\n\n");
				}
			}
